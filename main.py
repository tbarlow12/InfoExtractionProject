...
def answerType(questionType,person):
    return {
        1: person.summary,
        2: person.age,
        3: person.age
...
yearRegex = '(\\d+)'
monthRegex = '(January|February|March|April|June|July|August|September|October|November|December)'
...
def getDate(tup):
    monthString = tup[0].lower()
    month = getMonthNum(monthString)
    day = int(tup[1])
    year = int(tup[2])
...
def getAnswer(question):
    ...
    if (len(subject) > 0):
        ...
        #unicodedata.normalize('NFKD',summary).encode('ascii','ignore')
        content = page.content.encode('utf-8')
        sections = getSections(content)
        p = learnPerson(subject,summary,sections)
        return answerType(questionType,p)
